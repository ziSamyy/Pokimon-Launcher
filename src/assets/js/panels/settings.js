...
class Settings {
	...
	async initJavaPath() {
		...
		document.querySelector('.path-button').addEventListener('click', async () => {
			...
			if (
				file.value.replace('.exe', '').endsWith('java') ||
				file.value.replace('.exe', '').endsWith('javaw')
			) {
				this.database.update({ uuid: '1234', path: file.value }, 'java-path')
				path.value = file.value.replace(/\\/g, '/')
			} else alert('El nombre del archivo debe de ser java/w')
		})

		document.querySelector('.path-button-reset').addEventListener('click', () => {
			path.value = 'Usar versión de Java recomendada'
			file.value = ''
	...
	async initResolution() {
		...
		select.addEventListener('change', (event) => {
			...
			this.database.update(
				{ uuid: '1234', screen: { width: resolution[0], height: resolution[1] } },
				'screen'
			)
		})
	}
